...
int settings_load_from_file(char* path)
{
	...
	fprintf(stderr,"about to check access of balancebot settings file\n");
	#endif
	...
	PARSE_INT(mpu_sample_rate)
	PARSE_STRING(mpu_orientation)
	...
	PARSE_CONTROLLER(body_angle_controller)
	PARSE_CONTROLLER(position_controller)
	PARSE_CONTROLLER(steering_controller)
	PARSE_CONTROLLER(horiz_vel_ctrl)
